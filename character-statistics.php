...
if($characters->guid > 0 && $isCharacter && $armory->armoryconfig['useCache'] == true && !isset($_GET['skipCache'])) {
    if($achievement_category > 0) {
        $cache_id = $utils->GenerateCacheId('character-statistics-c'.$achievement_category, $characters->name, $armory->armoryconfig['defaultRealmName']);
    }
    else {
        $cache_id = $utils->GenerateCacheId('character-statistics', $characters->name, $armory->armoryconfig['defaultRealmName']);
    }
